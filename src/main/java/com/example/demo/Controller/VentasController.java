...
public class VentasController {
	...
	public String terminarVenta(Model model, @Validated Ventas v, HttpServletRequest request, RedirectAttributes redirectAttrs) {
		...
		for (ProductoParaVender productoParaVender : carrito) {
			...
			ProductosVentas productoVendido = new ProductosVentas(productoParaVender.getPrecio_venta(),
					productoParaVender.getCantidad(), productoParaVender.getTotal(), p, v);
			// Y lo guardamos
	...
	public String agregarAlCarrito(@ModelAttribute Productos producto, HttpServletRequest request,
			RedirectAttributes redirectAttrs) {
		ArrayList<ProductoParaVender> carrito = this.obtenerCarrito(request);
		// buscar por id
		Productos productoBuscadoPorID = productosRepository.findFirstById(producto.getId());
		if (productoBuscadoPorID == null) {
			redirectAttrs.addFlashAttribute("mensaje", "El producto con el ID " + producto.getId() + " no existe")
		...
		if (!encontrado) {
			carrito.add(new ProductoParaVender(productoBuscadoPorID.getId(), productoBuscadoPorID.getNombre(),
					productoBuscadoPorID.getPrecio_compra(),productoBuscadoPorID.getPrecio_venta(), productoBuscadoPorID.getStock(), 1));
